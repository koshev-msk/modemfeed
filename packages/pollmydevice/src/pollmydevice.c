...
int FormAuthAnswer(char *dataBuffer, long long int clientID);
...
{ 
    ...
    int yes = 1;
    if (setsockopt(threadFD->mainSocket, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) == -1)
    {
        LOG("Error while setting socket options, errno: %s\n", strerror(errno));
        pthread_exit(NULL);
    }
    ...
    {
        LOG("Error while reading port config %s; errno: %s\n", deviceConfig.deviceName, strerror(errno));
        pthread_exit(NULL);
    }
...
device_config_t GetFullDeviceConfig(int deviceID)
{
    ...
    char UCIpathClientID[MAX_CHARS_IN_UCIPATH]      = ".client_id";
...
int FormAuthAnswer(char *dataBuffer, long long int clientID)
{
    ...
    char stringClientID[15];
    sprintf(stringClientID, "%lld", clientID); // to string
    ...
    memcpy(&dataBuffer[6], &stringClientID[0], 15);   // copy imei to buffer
